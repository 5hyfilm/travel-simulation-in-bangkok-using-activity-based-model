...
public class RunMatsimApplication extends MATSimApplication {
	...
	protected Config prepareConfig(Config config) {
        ...
        var scoring = org.matsim.core.config.ConfigUtils
                .addOrGetModule(config, ScoringConfigGroup.class);

        addIfMissing(scoring, "residential", 12 * 3600);     // home-like
        addIfMissing(scoring, "commercial",  8 * 3600);      // work-like
        addIfMissing(scoring, "school",      6 * 3600);
        addIfMissing(scoring, "university",  6 * 3600);
        addIfMissing(scoring, "health",      6 * 3600);
        addIfMissing(scoring, "public_service", 8 * 3600);
        addIfMissing(scoring, "park",        2 * 3600);      // leisure
...
//		controler.addOverridingModule( new SimWrapperModule() ) ;
    ...
    private static void addIfMissing(ScoringConfigGroup scoring, String type, double typicalDur) {
        boolean exists = scoring.getActivityParams().stream()
                .anyMatch(p -> p.getActivityType().equals(type));
        if (!exists) {
            scoring.addActivityParams(
                    new ScoringConfigGroup.ActivityParams(type).setTypicalDuration(typicalDur)
            );
        }
    }
}
